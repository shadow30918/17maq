...
func=SubmitForm
接收登記者資料
input : t_name(登記者姓名), t_mobile (登記者手機), t_email (電子郵件)
-------------------------------------------------------------------------
output : OK(0=失敗), status(1=有欄位未填 / 2=有欄位格式不符 / 3=手機己登記,且仍在可領取時間內 / 4=己兌換)
output : fields(status=1 & 2 時才有, 列出那些欄位未填)
-------------------------------------------------------------------------
output : OK(1=成功), status(1=未登記過的登記者 / 2=手機己登記過,但己超過領取時間,所以可再次證記), t_no(登記者編號), t_sn_no(SN), token
=========================================================================
func=SentMms
發送簡訊
input : t_no(登記者編號), t_sn_no(SN), token (由 SubmitForm 成功時所得到的 token)
output : OK(0=失敗), status(1=有欄位沒填 / 2=有欄位格式不正確 / 3=token錯誤 / 4=查無該名登記者 / 5=簡訊間隔時間未到)
...
-------------------------------------------------------------------------
output : OK(1=成功),
output : info(array, [OK=0 && (status=3 || 4)] || OK=1) 包含 ==> name(登記者姓名), mobile(登記者手機), email(電子郵件), expire_date(兌換失效時間)
=========================================================================
func=StoreSubmit
櫃點兌換確認兌換
input : sn (透過網址取得)
...
//兌換頁活動網址
$exchange_url="https://www.shiseido-event.com/2017maquillage/exchange.html";
...
//if (strcmp($_SERVER['HTTP_HOST'],"www.shiseido-event.com")==0 || strcmp($_SERVER['HTTP_HOST'],"shiseido-event.com")==0) {	//只接受本機傳送資料
	if (isset($_GET['func'])) {
		...
		if ($json==null) {
			switch ($func) {
				case "SubmitForm":
					if (isset($_GET['t_name']) && isset($_GET['t_mobile']) && isset($_GET['t_email'])) {
						$arrInputFields=array('t_name'=>array('string'), 't_mobile'=>array('string',$mobileFormat), 't_email'=>array('string',$emailFormat));
						$t_name=$common->replaceParameter($_GET['t_name']);
						$t_mobile=$common->replaceParameter($_GET['t_mobile']);
						$t_email=$common->replaceParameter($_GET['t_email']);
						...
						if ($return_OK==1) {
							...
							fixExpireSingleTester($t_mobile, $event_no, $store_no);
						...
						if ($return_OK==0) {
							...
							$db->query($sqlStr);
							$db->bind(':event_no', $event_no, PDO::PARAM_INT);
							$db->bind(':c_no', $city_no, PDO::PARAM_INT);
							$db->bind(':s_no', $store_no, PDO::PARAM_INT);
							...
							//Trigger
							Trigger::TesterApply('AfterInsert', $event_no, $store_no, 0);
					...
					break;
				case "SentMms":
					if (isset($_GET['t_no']) && isset($_GET['t_sn_no']) && isset($_GET['token'])) {
						$arrInputFields=array('t_no'=>array('int'), 'token'=>array('string'));
						$t_no=$common->replaceParameter($_GET['t_no']);
						$t_sn_no=$common->replaceParameter($_GET['t_sn_no']);
						$token=$common->replaceParameter($_GET['token']);
						...
						//檢查資料庫 ==> Tester
						if ($return_OK==1) {
							$sqlStr="SELECT COUNT(t_no) AS total FROM Tester WHERE t_e_no=:event_no AND t_no=:t_no AND t_sn_no=:t_sn_no AND t_is_exchange=0 AND t_is_valid=1";
							$db->query($sqlStr);
							$db->bind(':event_no', $event_no, PDO::PARAM_INT);
							$db->bind(':t_no', $t_no, PDO::PARAM_INT);
							$db->bind(':t_sn_no', $t_sn_no, PDO::PARAM_INT);
				...
				case "StoreGetInfo":
					if (isset($_GET['sn'])) {
						$sn_no=$common->replaceParameter($_GET['sn']);

						$return_OK=1;
						$return_status=0;
						$arrInfo=array('name'=>'', 'mobile'=>'', 'expire_date'=>'');
						...
						//比對兌換碼, 確認兌換狀態
						if ($return_OK==1) {
							$sqlStr='SELECT t_no, t_name, t_mobile, t_email, t_is_exchange, t_is_valid, DATE_ADD(t_apply_date, INTERVAL '.EXCHANGE_EXPIRE_DATE.' DAY) as t_expire_date FROM Tester WHERE t_e_no=:event_no AND t_sn_no=:sn_no';
				...
				case 'StoreSubmit':
					if (isset($_GET['sn'])) {
						...
						if ($return_OK==0) {
							...
							//trigger
							Trigger::TesterExchangeAfterUpdate($event_no, $store_no, 0, 1, 0);
...
function fixExpireSingleTester($t_mobile,$e_no, $store_no) {
	global $event_no;
	$db=new Database();
	...
	if ($update_count>0) {	//有更新
		...
		$sqlStr='SELECT s_no, s_req_apply, s_req_exchange, (SELECT COUNT(t_no) FROM Tester WHERE t_e_no=s_e_no AND s_no=t_s_no AND t_is_valid=1) AS actual_req_apply, (SELECT COUNT(t_no) FROM Tester WHERE t_e_no=s_e_no AND s_no=t_s_no AND t_is_exchange=1) AS actual_req_exchange FROM Store WHERE s_no='.$store_no.' AND s_e_no='.$e_no;
		$db->query($sqlStr);
		$rows = $db->resultset();
