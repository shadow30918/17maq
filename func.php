...
//if (strcmp($_SERVER['HTTP_HOST'],"www.shiseido-event.com")==0 || strcmp($_SERVER['HTTP_HOST'],"shiseido-event.com")==0) {	//只接受本機傳送資料
	if (isset($_POST['func'])) {
		...
		if ($json==null) {
			switch ($func) {
				case "SubmitForm":
					if (isset($_POST['t_name']) && isset($_POST['t_mobile']) && isset($_POST['t_email'])) {
					...
					break;
				case "SentMms":
					if (isset($_POST['t_no']) && isset($_POST['t_sn_no']) && isset($_POST['token'])) {
						$arrInputFields=array('t_no'=>array('int'), 'token'=>array('string'));
						$t_no=$common->replaceParameter($_POST['t_no']);
						$t_sn_no=$common->replaceParameter($_POST['t_sn_no']);
